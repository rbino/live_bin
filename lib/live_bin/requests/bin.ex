defmodule LiveBin.Requests.Bin do
  use GenServer, restart: :temporary

  require Logger

  alias LiveBin.Config
  ...
  def handle_call({:append, request}, _from, state) do
    if state.count >= Config.bin_max_requests() do
      ...
      Phoenix.PubSub.broadcast(LiveBin.PubSub, "bins:#{state.bin_id}", :bin_size_exceeded)
  ...
  def handle_info(:timeout, state) do
    ...
    Phoenix.PubSub.broadcast(LiveBin.PubSub, "bins:#{state.bin_id}", :bin_timeout)
    {:stop, :shutdown, state}
  end
end
